...
public abstract class Interactable extends Sized
{
    ...
    public void onPropagateInput(InputState inputState)
    {
        ...
        if (rect.contains(inputState.posX, inputState.posY))
        {
            ...
            {
                hover = true;
                fireEvent(new UIEvent.MouseEnter(inputState, this));
            }
            if (inputState.leftDown)
            {
                ...
                {
                    hold = true;
                    fireEvent(new UIEvent.MousePress(inputState, this));
                }
            }
            else if (hold)
            {
                hover = false;
                hold = false;
                fireEvent(new UIEvent.MouseRelease(inputState, this));
            }
        }
        else if (hover)
        {
            hover = false;
            hold = false;
            fireEvent(new UIEvent.MouseLeave(inputState, this));
        }
    }
}
